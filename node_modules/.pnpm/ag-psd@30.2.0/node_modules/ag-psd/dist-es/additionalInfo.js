...
import { writeZeros, writeSignature, writeBytes, writeUint32, writeUint16, writeFloat64, writeUint8, writeInt16, writeFloat32, writeFixedPointPath32, writeUnicodeString, writeSection, writeUnicodeStringWithPadding, writeColor, writePascalString, writeInt32, writePattern } from './psdWriter';
...
function parseWarp(warp) {
    ...
    const result = Object.assign(Object.assign({ style: warpStyle.decode(warp.warpStyle) }, (warp.warpValues ? { values: warp.warpValues } : { value: warp.warpValue || 0 })), { perspective: warp.warpPerspective || 0, perspectiveOther: warp.warpPerspectiveOther || 0, rotate: Ornt.decode(warp.warpRotate), bounds: warp.bounds && {
...
function encodeWarp(warp) {
    ...
    const desc = Object.assign(Object.assign({ warpStyle: warpStyle.encode(warp.style) }, (warp.values ? { warpValues: warp.values } : { warpValue: warp.value || 0 })), { warpPerspective: warp.perspective || 0, warpPerspectiveOther: warp.perspectiveOther || 0, warpRotate: Ornt.encode(warp.rotate), bounds: /*1 ? { // testing
    ...
    if ('Fltr' in f) {
        switch (f.Fltr._classID) {
            ...
            case 'denoise': return Object.assign(Object.assign({}, base), { type: 'reduce noise', filter: {
                    ...
                    channelDenoise: f.Fltr.channelDenoise.map(c => (Object.assign({ channels: c.Chnl.map(Chnl.decode), amount: c.Amnt }, (c.EdgF ? { preserveDetails: c.EdgF } : {})))),
                } });
            ...
            case 'Crvs':
                {
                    return Object.assign(Object.assign({}, base), { type: 'curves', filter: Object.assign({ presetKind: presetKindType.decode(f.Fltr.presetKind) }, (f.Fltr.Adjs ? {
...
function serializeFilterFXItem(f) {
    ...
    switch (f.type) {
        ...
        case 'reduce noise': return Object.assign(Object.assign({}, base), { Fltr: {
                ...
                channelDenoise: f.filter.channelDenoise.map(c => (Object.assign({ _name: '', _classID: 'channelDenoiseParams', Chnl: c.channels.map(i => Chnl.encode(i)), Amnt: c.amount }, (c.preserveDetails ? { EdgF: c.preserveDetails } : {})))),
        ...
        case 'curves': return Object.assign(Object.assign({}, base), { Fltr: Object.assign({ _name: 'Curves', _classID: 'Crvs', presetKind: presetKindType.encode(f.filter.presetKind) }, (f.filter.adjustments ? {
...
addHandler('expA', adjustmentType('exposure'), (reader, target, left) => {
    ...
    target.adjustment = Object.assign(Object.assign({}, target.adjustment), { type: 'exposure', exposure: readFloat32(reader), offset: readFloat32(reader), gamma: readFloat32(reader) });
    skipBytes(reader, left());
}, (writer, target) => {
    const info = target.adjustment;
    writeUint16(writer, 1); // version
...
addHandler('mixr', adjustmentType('channel mixer'), (reader, target, left) => {
    ...
    const adjustment = target.adjustment = Object.assign(Object.assign({}, target.adjustment), { type: 'channel mixer', monochrome: !!readUint16(reader) });
