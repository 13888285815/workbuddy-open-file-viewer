...
__export(index_exports, {
  ...
  epubPlugin: () => epubPlugin,
  fallbackPlugin: () => fallbackPlugin,
  gisPlugin: () => gisPlugin,
  imagePlugin: () => imagePlugin,
  model3dPlugin: () => model3dPlugin,
  ofdPlugin: () => ofdPlugin,
...
function epubPlugin() {
  return {
    ...
    async render(ctx) {
      ...
      ctx.viewport.append(panel);
      try {
        const zip = await import_jszip.default.loadAsync(await readArrayBuffer(ctx.file));
...
function xpsPlugin() {
  return {
    ...
    async render(ctx) {
      ...
      ctx.viewport.append(panel);
      try {
        const zip = await import_jszip2.default.loadAsync(await readArrayBuffer(ctx.file));
...
function sanitizeHtml(html) {
  return import_dompurify2.default.sanitize(html, {
    USE_PROFILES: { html: true },
    ADD_ATTR: ["target"]
  });
}
    ...
    async render(ctx) {
      ...
      try {
        if (ext === "zip") {
          ...
          const decompressed = import_pako.default.ungzip(u8);
        ...
        try {
          ...
          if (subExt === "shp") {
            ...
            if (dbfEntry && shxEntry) {
              ...
              const newZip = new import_jszip5.default();
...
async function bunzip2(bytes) {
  ...
  try {
    const module2 = await import("seek-bzip");
    const decoder = module2.default || module2;
...
async function unxz(bytes) {
  const module2 = await import("xz-decompress");
  const XzReadableStream = module2.XzReadableStream || module2.default?.XzReadableStream;
...
function sanitizeEmailHtml(html) {
  return import_dompurify3.default.sanitize(html, {
    USE_PROFILES: { html: true },
    ADD_ATTR: ["target"],
...
var import_pako2 = __toESM(require("pako"), 1);
...
function decodeDrawioDiagram(value) {
  try {
    ...
    return import_pako2.default.inflateRaw(Uint8Array.from(decoded, (char) => char.charCodeAt(0)), {
      to: "string"
    });
  } catch {
    return value;
...
function createWasmPreview(bytes) {
  ...
  if (parsed.exports.length > 0) {
    const exports2 = createWasmList("\u5BFC\u51FA", parsed.exports.map((item) => `${item.name} \xB7 ${item.kind} #${item.index}`));
    preview.append(exports2);
  }
  return preview;
}
...
function parseWasm(bytes) {
  ...
  try {
    while (offset < bytes.length) {
      ...
      if (id === 0) {
        ...
        exports2.push(...parseWasmExports(bytes, payloadOffset, payloadEnd));
      }
  ...
  return { valid: true, version, sections, imports, exports: exports2 };
}
...
function parseWasmExports(bytes, offset, end) {
  ...
  for (let index = 0; index < count.value && offset < end; index++) {
    ...
    exports2.push({ name: exportName.value, kind: wasmExternalKind(kind), index: itemIndex.value });
  }
  return exports2;
}
