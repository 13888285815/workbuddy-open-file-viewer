...
const translations: Record<Language, Record<string, string>> = {
  zh: {
    ...
    "filemanager.title": "文件列表",
    "filemanager.search": "搜索文件...",
    "filemanager.empty": "暂无文件，请上传或选择示例",
    "filemanager.dropHint": "拖拽文件到此处",
    "filemanager.sortName": "按名称排序",
    "filemanager.sortSize": "按大小排序",
    "filemanager.sortType": "按类型排序",
    "filemanager.viewList": "列表视图",
    "filemanager.viewGrid": "缩略图视图",
    "filemanager.upload": "上传文件",
    "filemanager.clear": "清空列表",
    "filemanager.remove": "移除文件",
  ...
  en: {
    ...
    "filemanager.title": "File List",
    "filemanager.search": "Search files...",
    "filemanager.empty": "No files yet. Upload or select a sample.",
    "filemanager.dropHint": "Drop files here",
    "filemanager.sortName": "Sort by name",
    "filemanager.sortSize": "Sort by size",
    "filemanager.sortType": "Sort by type",
    "filemanager.viewList": "List view",
    "filemanager.viewGrid": "Grid view",
    "filemanager.upload": "Upload files",
    "filemanager.clear": "Clear list",
    "filemanager.remove": "Remove file",
    "playground.dropTitle": "Choose or drop files",
    "playground.dropDesc": "Multi-file preview queue supported",
...
// File Manager State
type SortType = "name" | "size" | "type";
type ViewType = "list" | "grid" | "tree";
let fileManagerSearchQuery = "";
let fileManagerSortType: SortType = "name";
let fileManagerViewType: ViewType = "tree";
let fileManagerFiles: Array<{ id: string; file: File | Blob | string; name: string; size: number; type: string }> = [];
let fileTreeRoot: FileTreeNode | null = null;
...
function generateFileId(): string {
  return `fm-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
}
...
function syncFilesToFileManager(files: Array<File | Blob | string>) {
  fileManagerFiles = files.map((file, index) => {
    const name = file instanceof File ? file.name : (typeof file === "string" ? inferFileNameFromSource(file) : "file");
  ...
  fileManagerFiles.forEach(file => {
    ...
    for (let i = 0; i < pathParts.length; i++) {
      ...
      if (isLast) {
        ...
        // This is a folder
        currentNode.children = currentNode.children || [];
        let folderNode = currentNode.children.find(child => child.isFolder && child.name === part);
        
        if (!folderNode) {
          folderNode = {
            id: `folder-${generateFileId()}`,
            name: part,
            path: currentPath,
            isFolder: true,
            children: []
...
function renderTreeNodes(node: FileTreeNode, depth: number): string {
  ...
  if (node.isFolder) {
    ...
    return `
      <div class="tree-node folder-node" data-id="${node.id}" data-depth="${depth}">
        <div class="tree-content" style="padding-left: ${paddingLeft + 8}px">
          <button class="tree-toggle ${isExpanded ? 'expanded' : 'collapsed'}">
            <svg><use href="${isExpanded ? '#icon-download' : '#icon-arrow'}"></use></svg>
          </button>
        ...
        ${isExpanded && node.children ? node.children.map(child => renderTreeNodes(child, depth + 1)).join('') : ''}
      </div>
    `;
  } else {
    // File node
    const globalIndex = fileManagerFiles.findIndex(f => f.id === node.id);
    const isActive = globalIndex === activeFileIndex;
    return `
      <div class="tree-node file-node ${isActive ? 'active' : ''}" data-id="${node.id}" data-depth="${depth}" data-index="${globalIndex}">
        <div class="tree-content" style="padding-left: ${paddingLeft + 8}px">
          <span class="tree-spacer"></span>
          <span class="tree-icon file-icon">${getFileIcon(node.type || '')}</span>
          <span class="tree-name">${node.name}</span>
          ${node.size ? `<span class="tree-size">${formatFileSize(node.size)}</span>` : ''}
        </div>
      </div>
    `;
  }
...
function attachTreeEventListeners() {
  ...
  fileManagerList.querySelectorAll('.file-node').forEach(node => {
    node.addEventListener('click', (e) => {
      const index = parseInt((node as HTMLElement).dataset.index || "0");
      selectFileFromManager(index);
    });
  });
}
...
function getFileIcon(extension: string): string {
  const iconMap: Record<string, string> = {
    'pdf': '<svg><use href="#icon-docs"></use></svg>',
    'doc': '<svg><use href="#icon-docs"></use></svg>',
    'docx': '<svg><use href="#icon-docs"></use></svg>',
    'xls': '<svg><use href="#icon-docs"></use></svg>',
    'xlsx': '<svg><use href="#icon-docs"></use></svg>',
    'ppt': '<svg><use href="#icon-docs"></use></svg>',
    'pptx': '<svg><use href="#icon-docs"></use></svg>',
    'jpg': '<svg><use href="#icon-image"></use></svg>',
    'jpeg': '<svg><use href="#icon-image"></use></svg>',
    'png': '<svg><use href="#icon-image"></use></svg>',
    'gif': '<svg><use href="#icon-image"></use></svg>',
    'svg': '<svg><use href="#icon-code"></use></svg>',
    'mp4': '<svg><use href="#icon-play"></use></svg>',
    'mp3': '<svg><use href="#icon-play"></use></svg>',
    'zip': '<svg><use href="#icon-archive"></use></svg>',
    'txt': '<svg><use href="#icon-code"></use></svg>',
    'json': '<svg><use href="#icon-code"></use></svg>',
    'md': '<svg><use href="#icon-code"></use></svg>',
  };
  return iconMap[extension.toLowerCase()] || '<svg><use href="#icon-file"></use></svg>';
}

function sortFileManagerFiles() {
  const searchLower = fileManagerSearchQuery.toLowerCase();
  let filtered = fileManagerFiles.filter(f =>
    f.name.toLowerCase().includes(searchLower)
  );

  filtered.sort((a, b) => {
    switch (fileManagerSortType) {
      case "name":
...
function renderFileManagerList() {
  ...
  if (fileManagerFiles.length === 0) {
    fileManagerList.innerHTML = `
      <div class="file-manager-empty">
        ...
        <p>${translations[language]["filemanager.empty"]}</p>
      </div>
    `;
    return;
  }
  ...
  // Tree view
  if (fileManagerViewType === "tree" && fileTreeRoot) {
    fileManagerList.innerHTML = renderTreeNodes(fileTreeRoot, 0);
    attachTreeEventListeners();
    return;
  }

  // List/Grid view (original functionality)
  const sorted = sortFileManagerFiles();

  fileManagerList.innerHTML = sorted.map((item, index) => {
    const globalIndex = fileManagerFiles.findIndex(f => f.id === item.id);
    const isActive = globalIndex === activeFileIndex;
    const itemClass = `file-manager-item${fileManagerViewType === "grid" ? " grid-view" : ""}${isActive ? " active" : ""}`;

    return `
      <div class="${itemClass}" data-index="${globalIndex}" data-id="${item.id}">
        <div class="fm-icon">
          <svg><use href="#icon-file"></use></svg>
        </div>
        <div class="fm-info">
          ...
          <div class="fm-meta">
            ${item.size > 0 ? `<span class="fm-size">${formatFileSize(item.size)}</span>` : ""}
            ${item.type ? `<span class="fm-type">${item.type}</span>` : ""}
          </div>
        </div>
        <button class="fm-delete" data-index="${globalIndex}" title="${translations[language]["filemanager.remove"]}">
  ...
  // Add click handlers
  fileManagerList.querySelectorAll(".file-manager-item").forEach(el => {
    el.addEventListener("click", (e) => {
      const target = e.target as HTMLElement;
      if (target.closest(".fm-delete")) return;
      const index = parseInt((el as HTMLElement).dataset.index || "0");
      selectFileFromManager(index);
    });
  });

  fileManagerList.querySelectorAll(".fm-delete").forEach(btn => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const index = parseInt((btn as HTMLElement).dataset.index || "0");
      removeFileFromManager(index);
    });
  });
}

function selectFileFromManager(index: number) {
  if (index < 0 || index >= fileManagerFiles.length) return;
  activeFileIndex = index;
  const item = fileManagerFiles[index];
  currentFiles = [item.file];
  ...
  if (vtFn) vtFn.textContent = item.name;
  renderFileManagerList();
}

function removeFileFromManager(index: number) {
  if (index < 0 || index >= fileManagerFiles.length) return;
  fileManagerFiles.splice(index, 1);
  if (activeFileIndex >= fileManagerFiles.length) {
    activeFileIndex = Math.max(0, fileManagerFiles.length - 1);
  }
  if (fileManagerFiles.length > 0) {
    currentFiles = [fileManagerFiles[activeFileIndex].file];
  } else {
    currentFiles = [];
  }
  syncFilesToFileManager(currentFiles);
  updateFilePickerLabel();
  renderViewer();
  const vtFn = document.getElementById("vtFileName");
  if (vtFn) {
    if (fileManagerFiles.length > 0) {
      vtFn.textContent = fileManagerFiles[activeFileIndex].name;
    } else {
      vtFn.textContent = "未选择文件";
    }
  }
  renderFileManagerList();
}
...
function initFileManager() {
  const vtFn = document.getElementById("vtFileName");
  if (vtFn && currentFileName) vtFn.textContent = currentFileName;
  syncFilesToFileManager(currentFiles);
  renderFileManagerList();

  // File search
  const fileSearchInput = document.getElementById("fileSearch") as HTMLInputElement;
  const searchClearBtn = document.getElementById("searchClear");
  if (fileSearchInput) {
    fileSearchInput.addEventListener("input", () => {
      fileManagerSearchQuery = fileSearchInput.value;
      renderFileManagerList();
    });
  ...
  // Sort buttons
  document.querySelectorAll(".vt-sort-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      const sortType = (btn as HTMLElement).dataset.sort as SortType;
      if (sortType) {
        fileManagerSortType = sortType;
        document.querySelectorAll(".vt-sort-btn").forEach(b => b.classList.remove("active"));
  ...
  if (fmUpload) {
    fmUpload.addEventListener("click", () => {
      fileInput.click();
    });
  }
  ...
  const vtFnEnd = document.getElementById("vtFileName");
  if (vtFnEnd && currentFileName) vtFnEnd.textContent = currentFileName;
}
